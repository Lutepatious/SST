...
#define PLANE 4
#define PATTERN 2
unsigned long chbuf[PATTERN][PLANE][ROW];

#define PNG_HEIGHT (ROW)
#define PNG_WIDTH (PATTERN*32)

void _cdecl main(int argc, char **argv);

unsigned __int32 vbuf[ROW][PATTERN][4];
...
void view(void)
{
	...
	{
		memset(&vbuf[y][pat], 0, 16);
		for (i = 0; i < PLANE; i++)
		{
			aimage = chbuf[pat][i][y];
			unsigned __int32 a4[4] = { 0, 0, 0, 0 };
			for (index = 0; index < 32; index++) {
				if (aimage & (1L << index)) {
					a4[index / 8] |= 1L << ((index % 8) * PLANE);
					vbuf[y][pat][0] |= _byteswap_ulong(a4[0]) << i;
					vbuf[y][pat][1] |= _byteswap_ulong(a4[1]) << i;
					vbuf[y][pat][2] |= _byteswap_ulong(a4[2]) << i;
					vbuf[y][pat][3] |= _byteswap_ulong(a4[3]) << i;
				}
			}
		}
	}
